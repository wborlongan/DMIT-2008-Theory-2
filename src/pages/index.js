...
export default function Home({ edmontonWeather, tokyoWeather }) {
  useEffect(() => {
  ...
  return (
    ...
    >
      <main className="text-white m-auto p-2 grid gap-2 max-w-6xl relative w-full sm:p-4 sm:gap-2 md:grid-cols-[1fr_1fr_0.9fr] md:gap-3 md:p-6 lg:grid-cols-4 lg:gap-4">
        ...
        {/* 🇨🇦 Edmonton Weather */}
        <WeatherCard title="Edmonton Weather" weather={edmontonWeather} />

        {/* 🇯🇵 Tokyo Weather */}
        <WeatherCard title="Tokyo Weather" weather={tokyoWeather} />
        ...
        <Card colSpan="md:col-span-1" rowSpan="md:row-span-1">
          <div className="relative min-h-[44px] overflow-hidden">
            <footer className="absolute inset-0 text-xs opacity-100 translate-y-0 transition-all duration-300 ease-out group-hover:-translate-y-3 group-hover:opacity-0 group-focus-within:-translate-y-3 group-focus-within:opacity-0">
              © {profileData.site.footer.year} ·{" "}
              {profileData.site.footer.prefix}{" "}
              <a
            ...
            >
              © 2024 · Originally Crafted with ♥️ using Astro by Gianmarco · See{" "}
              <a
                href="https://github.com/Ladvace/astro-bento-portfolio"
                target="_blank"
                rel="noreferrer"
                className="font-bold font-italic text-red-500"
              >
                here
              </a>
            </footer>
...
export async function getServerSideProps() {
  ...
  const tokyoWeather = await getWeatherForProfile(tokyoProfile, apiKey);
